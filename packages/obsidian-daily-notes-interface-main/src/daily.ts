...
import { getDateFromFile, getDateFromPath, getDateUID } from "./parse";
...
export function getAllDailyNotes(): Record<string, TFile> {
  ...
  Vault.recurseChildren(dailyNotesFolder, (note) => {
    if (note instanceof TFile) {
      const date = getDateFromPath(note.path.replace(`${folder}/`,''), "day");
      if (date) {
        const dateString = getDateUID(date, "day");
        dailyNotes[dateString] = note;
        note.displayName = note.path.replace(`${folder}/`,'').replace('.md','')
      }
    }
  });
